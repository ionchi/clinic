...
<head>
	...
	<title>Esami relativi al medico</title>
	<!-- Bootstrap -->
...
<div class="container content" style="background-color: #fff;">
	...
	<div align="center">
		<table class="table table-hover table-striped">
			...
			<tbody>
			<c:forEach var="exam" items="${doctorExamList}" varStatus="status">
				<tr>
					<td><b>${status.index + 1}</b></td>
					<td>${exam.creationDate}</td>
					<td>${exam.examDate}</td>
					<td>${exam.doctor.firstName}, ${exam.doctor.lastName}</td>
					<td>${exam.patient.firstName}, ${exam.patient.lastName}</td>
		...
		<div>
			<a href="${pageContext.request.contextPath}/admin"
			   class="btn btn-default">Torna all'area personale</a>
		</div>
		<br />
	</div>
</div>
